...
import OpenBlogIntrestedCard from "../bolgopen/OpenBlogIntrestedCard";
import Link from "next/link";

const FeatureBlog = () => {
  return (
    <>
      <div className="bg-[#FC8B99] ">
        <div className="max-w-[1120px] w-full mx-auto px-3">
          <h1 className="sm:py-8 py-4 text-center text-[#313131] font-Montserrat text-[32px] font-extrabold leading-normal">
            Featured blogs
          </h1>
      ...
      <div className="max-w-[1120px] pb-[100px] w-full mx-auto px-4 xl:px-0">
        <div className="py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 lg:gap-9 xl:gap-[68px] ">
          {NewsFeatureBlogSecond.map((item, index) => (
            <OpenBlogIntrestedCard item={item} key={index} />
          ))}
        </div>
      </div>
